...
public class BoardDetailAction implements Action{
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		BoardRepository boardRepository =
				BoardRepository.getInstance(); // Board 
		ReplyRepository replyRepository = 
				ReplyRepository.getInstance(); // Reply
		// 조회수 증가가 상세보기가 되기 전에 실행되는 것이 좋음 
		int result = boardRepository.updateReadCount(boardId);
		System.out.println("BOARD READCOUNT RESULT : " + result);
		if(result != 1) {
			Script.back("서버 오류!", response);
			return;
		}
		// Board, User (해당 게시물의 글과 글 작성자)
		BoardResponseDto boardDto =
				boardRepository.findById(boardId);
		// Reply, User (해당 게시물의 댓글과 댓글 작성자) 복수
